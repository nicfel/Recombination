...
public class BubbelOperator extends DivertLociOperator {
...
//            System.out.println(logHR);
    ...
    double addRecombination() {
        ...
        RecombinationNetworkEdge destEdge = sourceEdge;
...
//        }
        ...
        sourceEdge=sourceEdge.parentNode.getParentEdges().get(0);
        
        if (sourceEdge.breakPoints.getLength()<2)
            return Double.NEGATIVE_INFINITY;
        ...
        minDestTime = Math.max(destEdge.childNode.getHeight(), sourceTime);
        ...
        int nRemovableEdges = (int) network.getEdges().stream()
		      .filter(e -> !e.isRootEdge())
  		      ...
  		      .filter(e -> e.childNode.otherParentEdge(e).parentNode.isCoalescence())
  		      .filter(e -> e.childNode.otherParentEdge(e).parentNode.otherChildEdge(e.childNode.otherParentEdge(e)).childNode.ID==e.childNode.getChildEdges().get(0).childNode.ID)
		      .count();
...
//                .collect(Collectors.toList());
        ...
        logHR += Math.log(1.0/(sourceEdge.parentNode.getParentEdges().get(0).parentNode.getHeight()-sourceEdge.childNode.getHeight()));
        logHR += Math.log(1.0/(sourceEdge.parentNode.getParentEdges().get(0).parentNode.getHeight()-sourceEdge.childNode.getChildEdges().get(0).childNode.getHeight()));

        // Remove source edge
        logHR += removeRecombinationEdge(sourceEdge);
